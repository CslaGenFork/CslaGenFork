...
if (CurrentUnit.GenerationParams.GenerateAsynchronous || CurrentUnit.GenerationParams.GenerateSilverlight4)
{
    foreach (Criteria c in Info.CriteriaObjects)
    {
        ...
        Public Shared Sub Delete<%= Info.ObjectName %><%= c.GetOptions.FactorySuffix %>(crit As <%= c.Name %>, callback As EventHandler(Of DataPortalResult(Of <%= Info.ObjectName %>)))
            DataPortal.BeginDelete(Of <%= Info.ObjectName %>)(crit, callback)
        End Sub
        <%
            }
            %>
...
<%
        ...
        <%= Info.ParentType == string.Empty ? "Public" : "Friend" %> Shared Sub Delete<%= Info.ObjectName %><%= c.DeleteOptions.FactorySuffix %>(<%= strDelParams %>)
            ...
            {
                %>DataPortal.BeginDelete(Of <%= Info.ObjectName %>)(New <%= c.Name %>(<%= strDelCritParams %>), callback)<%
            }
            else if (c.Properties.Count > 0)
            {
                %>DataPortal.BeginDelete(Of <%= Info.ObjectName %>)(<%= SendSingleCriteria(c, strDelCritParams) %>, callback)<%
            }
            else
            {
