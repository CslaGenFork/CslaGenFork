    ...
    public class CslaTemplateHelperVB : CodeTemplate
    {
        ...
        public virtual string GetDataSetStatement(ValueProperty prop)
        {
            ...
            if (nullable)
            {
                if (prop.PropertyType.IsNullableType())
                    statement += String.Format("If(Not dr.IsDBNull(\"{0}\"), New {1}(({2}) ",
                        prop.ParameterName,
                        GetDataType(prop),
                        GetDataType(prop.PropertyType));
                else
                    statement += String.Format("IF(Not dr.IsDBNull(\"{0}\"), ({1})",
                        prop.ParameterName,
                        GetDataType(prop));
            }
        ...
        public virtual string GetDataReaderStatement(ValueProperty prop)
        {
            ...
            if (nullable)
            {
                ...
                {
                    statement += String.Format("If(dr.IsDBNull(\"{0}\"), Nothing, ", prop.ParameterName);
                    ifNull = true;
                }
            }
            statement += "dr.";
        ...
        public virtual string GetParameterSet(Property prop, bool useCrit, bool singleCriteria, bool useField)
        {
            ...
            if (nullable)
            {
                ...
                if (propType == TypeCodeEx.SmartDate)
                    return String.Format("If({0}.IsEmpty, DBNull.Value, {0})", propName);
        ...
        public static string GetDataType(ValueProperty prop)
        {
        ...
        public static string GetBackingDataType(ValueProperty prop)
        {
        ...
        public string GetUsingStatementsStringBusiness(CslaObjectInfo info, CslaGeneratorUnit unit)
        {
            ...
            result = info.GetAllValueProperties()
                ...
                .Aggregate(result,
                    (current, p) =>
                        current + ("Imports System.Linq 'Added for Byte[] helpers" + Environment.NewLine));
        ...
        public string ReturnParameterValue(BusinessRuleConstructorParameter parameter)
        {
            ...
            else if (parameter.Type == "List<String>" && result != string.Empty)
            {
                ...
                result = "New List(Of String) From {" + result + "}";
            }
            else if (parameter.Type == "List<Int32>" && result != string.Empty)
            {
                ...
                result = "New List(Of Integer) From {" + result + "}";
            }
        ...
        private string PropertyInfoUoWDeclare(CslaObjectInfo info, UnitOfWorkProperty prop, bool isSilverlight)
        {
            ...
            var response =
                String.Format(
                    "{0} Shared ReadOnly {2} As PropertyInfo(Of {1}) = RegisterProperty(Of {1})(Function(p) p.{3}, \"{4}\")",
                    PropertyInfoVisibility(),
                    prop.TypeName,
                    FormatPropertyInfoName(prop.Name),
                    FormatPascal(prop.Name),
        ...
        public string PropertyConvertDeclare(CslaObjectInfo info, ConvertValueProperty prop)
        {
            ...
            response += String.Format("{0}{1} Property {3} As {2}" + Environment.NewLine,
                GetPropertyAccess(prop),
                (isReadOnly ? " ReadOnly" : ""),
                GetDataTypeGeneric(prop, prop.PropertyType),
                FormatPascal(prop.Name));
            ...
            response += "                If " +
                        (sourceProperty.Nullable ? sourceProperty.Name + ".HasValue AndAlso " : string.Empty) +
                        prop.NVLConverter + "().ContainsKey(" + prop.SourcePropertyName +
                        (sourceProperty.Nullable ? ".Value" : string.Empty) + ") Then" + Environment.NewLine;
            ...
            if (!isReadOnly)
            {
                ...
                response += "                If " + prop.NVLConverter + "().ContainsValue(value) Then" +
                            Environment.NewLine;
                response += "                    Dim result = " + prop.NVLConverter + "().GetItemByValue(value).Key" +
                            Environment.NewLine;
                response += "                    If result <> " + prop.SourcePropertyName + " Then" +
                            Environment.NewLine;
        ...
        private string PropertyDeclareGetter(ValueProperty prop)
        {
            ...
            {
                response +=
                    String.Format("            Get{3}                Return {0}{1}({2}){3}            End Get{3}",
                        isConversion
                            ? "GetPropertyConvert"
                            : "GetProperty",
                        isConversion
                            ? "(Of " + GetDataType(prop.BackingFieldType) + ", " + GetDataType(prop) + ")"
                            : "",
                        (prop.DeclarationMode == PropertyDeclaration.Unmanaged ||
                         prop.DeclarationMode == PropertyDeclaration.UnmanagedWithTypeConversion)
            ...
            string convertPropertyName)
        {
                ...
                prop.DeclarationMode == PropertyDeclaration.UnmanagedWithTypeConversion)
            {
                response += String.Format(
                    "            {0}Set(ByVal value As {1}){6}                {2}{3}({4}, value){5}{6}            End Set{6}",
            ...
            {
                response += String.Format(
                    "            {0}Set(ByVal value As {1}){4}                {2} = value{3}{4}            End Set{4}",
                    PropertyDeclareSetterVisibility(isReadOnly, prop),
                    //GetDataTypeGeneric(prop, prop.PropertyType),
                    GetDataType(prop),
        ...
        private string ChildPropertyDeclareGetter(CslaObjectInfo info, ChildProperty prop)
        {
            ...
            {
                response =
                    String.Format("            Get{1}                Return GetProperty({0}){1}            End Get{1}",
                        FormatPropertyInfoName(prop.Name),
                        Environment.NewLine);
            }
            else //if (prop.DeclarationMode == PropertyDeclaration.ClassicProperty)
        ...
        private string ChildPropertyDeclareSetter(bool isReadOnly, ChildProperty prop)
        {
            ...
            {
                response += String.Format(
                    "            {0}Set(ByVal value As {1}){4}                {2}({3}, value){4}            End Set{4}",
                    ChildPropertyDeclareSetterVisibility(isReadOnly, prop),
                    prop.TypeName,
            ...
            {
                response += String.Format(
                    "            {0}Set(ByVal value As {1}){3}                {2} = value{3}            End Set{3}",
                    ChildPropertyDeclareSetterVisibility(isReadOnly, prop),
                    prop.TypeName,
                    FormatFieldName(prop.Name),
                    Environment.NewLine);
        ...
        private string EditableChildLazyLoadAsync(CslaObjectInfo info, ChildProperty prop, bool isLocal)
        {
            ...
            var result = string.Empty;
            result += String.Format("                If Not FieldManager.FieldExists({0}) Then" + Environment.NewLine,
                FormatPropertyInfoName(prop.Name));
            result += String.Format("                    LoadProperty({0}, Nothing)" + Environment.NewLine,
                FormatPropertyInfoName(prop.Name));
            result += "                    If Me.IsNew Then" + Environment.NewLine;
            if (UseChildFactoryHelper)
                result += String.Format("                        {0}.New{0}(Function(o, e)" + Environment.NewLine,
                    prop.TypeName);
            else
                result +=
                    string.Format(
                        "                        DataPortal.BeginCreate(Of {0})(Function(o, e)" + Environment.NewLine,
                        prop.TypeName);
            ...
            if (!UseChildFactoryHelper && isLocal)
                result += "                            End Function, DataPortal.ProxyModes.LocalOnly)" +
                          Environment.NewLine;
            else
        ...
        private string ReadOnlyChildLazyLoadAsync(CslaObjectInfo info, ChildProperty prop, bool isLocal)
        {
            ...
            else
            {
                result +=
                    string.Format(
                        "                    DataPortal.BeginFetch(Of {0})({1}, Function(o, e)" + Environment.NewLine,
                        prop.TypeName, GetFieldReaderStatementList(info, prop));
            }
            result += "                            if e.Error IsNot Nothing Then" + Environment.NewLine;
            result += "                                Throw e.Error" + Environment.NewLine;
        ...
        public string UnitOfWorkPropertyDeclare(CslaObjectInfo info, UnitOfWorkProperty uowProp)
        {
            ...
            if (uowProp.DeclarationMode == PropertyDeclaration.Managed)
            {
                ...
                response += String.Format(
                    "            {0}Set(ByVal value As {3}){2}                LoadProperty({1}, value){2}            End Set{2}",
                    isReadOnly ? "Private " : "",
                    FormatPropertyInfoName(uowProp.Name),
                    Environment.NewLine,
                    uowProp.TypeName);
        ...
        public string GetCommand(CslaObjectInfo info, string commandText, bool useInlineQueries, string criteria)
        {
            if (useInlineQueries)
            {
                ...
                result += Environment.NewLine + "                ";
                result +=
                    string.Format("Using cmd = New SqlCommand({0}InlineQuery, {1})",
        ...
        public string GetCommand(CslaObjectInfo info, string commandText)
        {
            ...
            return "Using cmd = New SqlCommand(\"" + plainTableSchema + commandText + "\", " +
                   LocalContextConnection(info) + ")";
        }
            ...
            if (UseSilverlight())
            {
                switch (step)
                {
                    case Conditional.Silverlight:
                        result = (formFeedBefore ? Environment.NewLine : "") + "#If SILVERLIGHT Then" +
                                 (formFeedAfter ? Environment.NewLine : "") + new string(' ', indent*4);
                        outputValue = silverlightLevel + 1;
                        break;
                    case Conditional.NotSilverlight:
                        result = (formFeedBefore ? Environment.NewLine : "") + "#If Not SILVERLIGHT Then" +
                                 (formFeedAfter ? Environment.NewLine : "") + new string(' ', indent*4);
                        outputValue = silverlightLevel + 1;
                        break;
                    ...
                    case Conditional.End:
                        result = (formFeedBefore ? Environment.NewLine : "") + "#End If" +
