...
namespace ParentLoad.Business.ERCLevel
{
    ...
    public partial class B01_ContinentColl : BusinessListBase<B01_ContinentColl, B02_Continent>
    {
        ...
        {
            foreach (var b02_Continent in this.DeletedList)
            {
                if (b02_Continent.Continent_ID == continent_ID)
                {
                    return true;
        ...
        private void LoadCollection(IB01_ContinentCollDal dal)
        {
            if (this.Count > 0)
        ...
        /// <summary>
        /// Loads all <see cref="B01_ContinentColl"/> collection items from the given list of B02_ContinentDto.
        /// </summary>
        /// <param name="data">The list of <see cref="B02_ContinentDto"/>.</param>
        private void Fetch(List<B02_ContinentDto> data)
        {
            var rlce = RaiseListChangedEvents;
            RaiseListChangedEvents = false;
            foreach (var dto in data)
            {
                Add(B02_Continent.GetB02_Continent(dto));
            }
            RaiseListChangedEvents = rlce;
        }
