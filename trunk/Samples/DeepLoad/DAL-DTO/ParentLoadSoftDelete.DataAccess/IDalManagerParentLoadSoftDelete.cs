//-----------------------------------------------------------------------
// <copyright file="IDalManagerParentLoadSoftDelete.cs" company="Marimer LLC">
//     Copyright (c) Marimer LLC. All rights reserved.
//     Website: http://www.lhotka.net/cslanet/
// </copyright>
//-----------------------------------------------------------------------

using System;

namespace ParentLoadSoftDelete.DataAccess
{
    /// <summary>
    /// Defines the ParentLoadSoftDelete DAL manager interface for DAL providers.
    /// </summary>
    public interface IDalManagerParentLoadSoftDelete : IDisposable
    {
        /// <summary>
        /// Gets the DAL provider for a given object Type.
        /// </summary>
        /// <typeparam name="T">Object Type that requires a ParentLoadSoftDelete DAL provider.</typeparam>
        /// <returns>A new ParentLoadSoftDelete DAL instance for the given Type.</returns>
        T GetProvider<T>() where T : class;
    }
}
