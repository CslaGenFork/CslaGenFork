/****** Object:  StoredProcedure [AddF04Level11] ******/
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[AddF04Level11]') AND type in (N'P', N'PC'))
    DROP PROCEDURE [AddF04Level11]
GO

CREATE PROCEDURE [AddF04Level11]
...
/****** Object:  StoredProcedure [UpdateF04Level11] ******/
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[UpdateF04Level11]') AND type in (N'P', N'PC'))
    DROP PROCEDURE [UpdateF04Level11]
GO

CREATE PROCEDURE [UpdateF04Level11]
    @Level_1_1_ID int,
    @Level_1_1_Name varchar(50)
AS
    BEGIN
        ...
        BEGIN
            RAISERROR ('''F04Level11'' object not found. It was probably removed by another user.', 16, 1)
            RETURN
        END
...
/****** Object:  StoredProcedure [DeleteF04Level11] ******/
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[DeleteF04Level11]') AND type in (N'P', N'PC'))
    DROP PROCEDURE [DeleteF04Level11]
GO

CREATE PROCEDURE [DeleteF04Level11]
    @Level_1_1_ID int
AS
    BEGIN
        ...
        /* Mark F04Level11 object as not active */
        UPDATE [Level_1_1]
        SET    [IsActive] = 'false'
        WHERE
            [Level_1_1_ID] = @Level_1_1_ID
