...
namespace Invoices.DataAccess.SQLite
{
    ...
    public partial class SupplierProductItemDal : ISupplierProductItemDal
    {
        ...
        public SupplierProductItemDto Insert(SupplierProductItemDto supplierProductItem)
        {
            ...
            {
                using (var cmd = new SQLiteCommand("dbo.AddSupplierProductItem", ctx.Connection))
                {
        ...
        public SupplierProductItemDto Update(SupplierProductItemDto supplierProductItem)
        {
            using (var ctx = ConnectionManager<SQLiteConnection>.GetManager("Invoices"))
            {
                using (var cmd = new SQLiteCommand("dbo.UpdateSupplierProductItem", ctx.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public void Delete(int productSupplierId)
        {
            using (var ctx = ConnectionManager<SQLiteConnection>.GetManager("Invoices"))
            {
                using (var cmd = new SQLiteCommand("dbo.DeleteSupplierProductItem", ctx.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
