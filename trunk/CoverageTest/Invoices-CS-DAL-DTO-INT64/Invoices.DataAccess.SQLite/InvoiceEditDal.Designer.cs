...
namespace Invoices.DataAccess.SQLite
{
    ...
    public partial class InvoiceEditDal : IInvoiceEditDal
    {
        ...
        public InvoiceEditDto Fetch(Guid invoiceId)
        {
            ...
            {
                using (var cmd = new SQLiteCommand("dbo.GetInvoiceEdit", ctx.Connection))
                {
        ...
        public InvoiceEditDto Insert(InvoiceEditDto invoiceEdit)
        {
            using (var ctx = ConnectionManager<SQLiteConnection>.GetManager("Invoices"))
            {
                using (var cmd = new SQLiteCommand("dbo.AddInvoiceEdit", ctx.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    ...
                    cmd.Parameters.Add("@NewRowVersion", DbType.DateTime2).Direction = ParameterDirection.Output;
                    cmd.ExecuteNonQuery();
        ...
        public InvoiceEditDto Update(InvoiceEditDto invoiceEdit)
        {
            using (var ctx = ConnectionManager<SQLiteConnection>.GetManager("Invoices"))
            {
                using (var cmd = new SQLiteCommand("dbo.UpdateInvoiceEdit", ctx.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@InvoiceId", invoiceEdit.InvoiceId).DbType = DbType.Guid;
                    cmd.Parameters.AddWithValue("@InvoiceNumber", invoiceEdit.InvoiceNumber).DbType = DbType.String;
        ...
        public void Delete(Guid invoiceId)
        {
            using (var ctx = ConnectionManager<SQLiteConnection>.GetManager("Invoices"))
            {
                using (var cmd = new SQLiteCommand("dbo.DeleteInvoiceEdit", ctx.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@InvoiceId", invoiceId).DbType = DbType.Guid;
                    var rowsAffected = cmd.ExecuteNonQuery();
