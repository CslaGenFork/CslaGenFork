...
namespace Invoices.DataAccess.SQLite
{
    ...
    public partial class ProductTypeEditDal : IProductTypeEditDal
    {
        ...
        public ProductTypeEditDto Fetch(int productTypeId)
        {
            ...
            {
                using (var cmd = new SQLiteCommand("dbo.GetProductTypeEdit", ctx.Connection))
                {
        ...
        public ProductTypeEditDto Insert(ProductTypeEditDto productTypeEdit)
        {
            using (var ctx = ConnectionManager<SQLiteConnection>.GetManager("Invoices"))
            {
                using (var cmd = new SQLiteCommand("dbo.AddProductTypeEdit", ctx.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public ProductTypeEditDto Update(ProductTypeEditDto productTypeEdit)
        {
            using (var ctx = ConnectionManager<SQLiteConnection>.GetManager("Invoices"))
            {
                using (var cmd = new SQLiteCommand("dbo.UpdateProductTypeEdit", ctx.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public void Delete(int productTypeId)
        {
            using (var ctx = ConnectionManager<SQLiteConnection>.GetManager("Invoices"))
            {
                using (var cmd = new SQLiteCommand("dbo.DeleteProductTypeEdit", ctx.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@ProductTypeId", productTypeId).DbType = DbType.Int32;
                    var rowsAffected = cmd.ExecuteNonQuery();
