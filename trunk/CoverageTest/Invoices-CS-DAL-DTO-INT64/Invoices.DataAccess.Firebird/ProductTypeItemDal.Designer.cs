...
namespace Invoices.DataAccess.Firebird
{
    ...
    public partial class ProductTypeItemDal : IProductTypeItemDal
    {
        ...
        public ProductTypeItemDto Insert(ProductTypeItemDto productTypeItem)
        {
            ...
            {
                using (var cmd = new FbCommand("dbo.AddProductTypeItem", ctx.Connection))
                {
        ...
        public ProductTypeItemDto Update(ProductTypeItemDto productTypeItem)
        {
            using (var ctx = ConnectionManager<FbConnection>.GetManager("Invoices"))
            {
                using (var cmd = new FbCommand("dbo.UpdateProductTypeItem", ctx.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public void Delete(int productTypeId)
        {
            using (var ctx = ConnectionManager<FbConnection>.GetManager("Invoices"))
            {
                using (var cmd = new FbCommand("dbo.DeleteProductTypeItem", ctx.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
